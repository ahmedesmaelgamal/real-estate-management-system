    ...
    <div class="row">
        ...
        <div class="col-md-12 col-lg-12">
            <div class="">
                <div class="d-flex justify-content-between">
                    <h2 class="fw-bold" style="color: #00193a; margin-bottom: 0px;">{{ trns('users') }}</h2>
                    <div class=" d-flex">
                        ...
                        @can('create_user')
                            <button class="btn btn-icon text-white addBtn" data-type="create" style="border: none;">
                                ...
                                </span> {{ trns('add_new_owner') }}
                            </button>
                        @endcan

                        <button
                            class="btn btn-icon {{ lang() == 'ar' ? 'me-2' : 'ms-2' }} text-white addExcelFile importBtn">
                            <span>
        ...
        <div class="modal fade" id="editOrCreate" data-backdrop="static" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="example-Modal3">{{ trns('add_new_owner') }}</h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
...
@section('ajaxCalls')
    ...
    <script>
        ...
        const showRoute = @json(route($route . '.show', ['user' => ':id']));
    ...
    <script>
        $(document).on('click', '.exportExcel', function() {
            ...
            let routeOfShow = '{{ route('users.export.excel') }}';
        ...
        $(document).on('click', '.addExcelFile', function() {
            ...
            let routeOfShow = '{{ route('users.add.excel') }}';
        ...
        function initExcelForm() {
            $('#excel-import-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        let errorMessage = '{{ trns('server_connection_error') }}';

                        if (xhr.responseJSON?.message) {
                            errorMessage = xhr.responseJSON.message
                                .map(e => e.errors.map(err => `صف ${e.row}: ${err}`).join('\n'))
                                .join('\n');
                        }
