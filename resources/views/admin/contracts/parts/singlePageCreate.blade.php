...
@section('title')
    {{ config()->get('app.name') }} | {{ trns('create_contract') }}
@endsection

@section('page_name')
    {{ trns('create_contract') }}
@endsection

@section('content')

    <div class="p-5">
        <h2 class="fw-bold">{{ trns('create_new_contract') }}</h2>
        @if ($errors->any())
        ...
        <form method="POST" id="addContractForm" enctype="multipart/form-data" action="{{ $storeRoute }}">
            @csrf
            <div class="row">
                ...
                <div class="col-4">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="contract_type" id="contract_type">
                            <option value="general" selected>{{ trns('general_contract') }}</option>
                            <option value="association">{{ trns('association') }}</option>
                ...
                <div class="col-4" id="association_Detail_name">
                ...
                <div class="col-4" id="association_Detail_number">
                    <label class="form-control-label " style=" color: black;">
                ...
                <div class="col-4" id="association_Detail_establish">
                    <label class="form-control-label " style=" color: black;">
                        {{ trns('establish_date') }}</label>
                    <input type="text" class="form-control" disabled value="{{ old('association_establish_date') }}"
                        name="association_establish_date" id="association_establish_date"
                        placeholder="{{ trns('association_establish_date') }}">
                </div>
                {{-- </div> --}}
                {{-- First Party --}}
                ...
                <div class="col-4" id="first_association_user" style="display: none;">
                    <div class="form-group">
                        <label for="first_association_user_id"
                            class="form-control-label">{{ 1 . ' ' . trns('user') }}<span
                                class="text-danger"></span></label>
                ...
                <div class="col-4" id="second_association_user" style="display: none;">
                    <div class="form-group">
                        <label for="second_association_user_id"
                            class="form-control-label">{{ 2 . ' ' . trns('user') }}<span
                                class="text-danger"></span></label>
                            ...
                            @foreach ($contractTerms as $contractTerm)
                                <div class="col-12">
                                    <div class="card custom-checkbox-card ">
                                        <input class="form-check-input d-none" checked type="checkbox"
                                            name="contract_term_id[]" id="template{{ $loop->index + 1 }}"
                                            value="{{ $contractTerm->id }}"
...
@section('ajaxCalls')
        ...
        $(document).ready(function() {
            $('#contract_type').on('change', function() {
                ...
                if (selectedType === 'association') {
                    ...
                    $("#association_Detail_name").show();
                    $("#association_Detail_establish").show();
                    $("#association_Detail_number").show();
                    ...
                    $("#association_Detail_name").hide();
                    $("#association_Detail_establish").hide();
                    $("#association_Detail_number").hide();
                    $("#association_admin").hide();
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#contract_party_two_id').select2({
                width: '100%'
            });
            $('#contract_party_id').select2({
                width: '100%'
            });
            $('#contract_name_id').select2({
                width: '100%'
            });
            $('#contract_location_id').select2({
                width: '100%'
            });
        ...
        $(document).on('shown.bs.modal', '#editOrCreate', function() {
    ...
    <script>
        $(document).on('click', '.createTerm', function(e) {
            ...
            setTimeout(function() {
                $('#modal-body').load('{{ route('terms.create') }}');
            }, 250);
        });
                ...
                success: function(data) {
                    ...
                    <div class="col-12 mb-3">
                        <div class="card custom-checkbox-card">
                            ...
                            <label for="template${term.id}" class="card-body text-right w-100">
                                <h5 class="association-card-header" style="font-weight: bold;">${term.title}</h5>
                                <p class="association-card-para" style="color: #b5b5c3;">${term.description}</p>
                            </label>
                        </div>
                    </div>
                `;
