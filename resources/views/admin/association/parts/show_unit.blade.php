...
@section('title')
    {{ config()->get('app.name') }} | {{ trns('show_unit_number') . '( ' . $bladeName . ' )' }}
@endsection
@section('page_name')
    {{ trns('show_unit_number') . '( ' . $bladeName . ' )' }}
@endsection
@section('content')
    <style>
        .gallery-image {
    ...
    <div class="row">
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card p-15" style="padding: 25px ; padding-bottom: 60px">
                <button data-v-268c4d9a="" type="button" class="btn go-back btn-secondary"
                    ...
                    onclick="window.location.href='/admin/association/real_states/getData/{{ $obj->RealState->id }}';">
                ...
                <div class="show-content mt-3">
                    ...
                    <div class="row m-4">
                        ...
                        {{--                        </div> --}}
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('unit_number') }}</h4>
                            <p>{{ $obj->unit_number }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('real_state_number') }}</h4>
                            <p>{{ $obj->RealState?->real_state_number }}</p>
                        </div>

                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('association_name') }}</h4>
                            <p>{{ $obj->RealState?->association->name }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('real_state_name') }}</h4>
                            <p>{{ $obj->RealState?->name }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('real_state_name') }}</h4>
                            <p>{{ $obj->RealState?->name }}</p>
                        ...
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('unified_code') }}</h4>
                            <p>{{ $obj->unified_code }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('floor_count') }}</h4>
                            <p>{{ $obj->floor_count }}</p>
                        ...
                        <div class="col-lg-2 col-md-2 mb-4">
                            ...
                            <p>{{ $obj->bathrooms_count }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('bedrooms_count') }}</h4>
                            <p>{{ $obj->bedrooms_count }}</p>
                        </div>

                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('northern_border') }}</h4>
                            <p>{{ $obj->northern_border }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('eastern_border') }}</h4>
                            <p>{{ $obj->eastern_border }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('western_border') }}</h4>
                            <p>{{ $obj->western_border }}</p>
                        </div>
                        <div class="col-lg-2 col-md-2 mb-4">
                            <h4> {{ trns('northern_border') }}</h4>
                            <p>{{ $obj->northern_border }}</p>
                        ...
                        <div class="col-lg-2 col-md-2 mb-4"
                            style="background-color: {{ $obj->status == 1 ? '#40E0D0' : '#FF6347' }};">
                            <h4> {{ trns('status') }}</h4>
                            <p>{{ $obj->status == 1 ? 'نشط' : 'غير نشط' }}</p>
                        </div>
                    </div>
                </div>
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $('.usersTable tbody').on('click', 'tr:not(.details-row)', function(e) {
                        if ($(e.target).is(
                                'input, button, a, .delete-checkbox, :has(input, button, a, .delete-checkbox)')) {
                            return;
                        }
        ...
        // `;
                ...
                $('#dataTable tbody').on('click', 'tr:not(.details-row)', function(e) {
                    ...
                    // Redirect to the details page
                    // window.location.href = `/admin/association/units/getData/${id}`;
                    window.location.href = "{{ route('association.units.getData', '') }}/" + id;
